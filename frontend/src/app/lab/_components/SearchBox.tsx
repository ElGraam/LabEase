...
import { ChevronDownIcon, SearchIcon, TriangleDownIcon } from '@chakra-ui/icons';
import {
  baseTheme,
  Box,
  Button,
...
const SearchBox = ({isBookmark} : {isBookmark: boolean}) => {
  ...
  // 並び替えの状態を管理
  const [sortOrder, setSortOrder] = useState<string>(searchParams?.get('sort') || 'new');
    ...
    <Box sx={FormStyle}>
      <form className='searchForm' action={onSubmit}>
        <InputGroup sx={FormStyle}>
          ...
          />
          <Button type='submit' variant='outline' aria-label='Search Lab' sx={IconButtonStyle} mr={4}>
            <SearchIcon />
          </Button>
          {!isBookmark && 
          <Menu>
            ...
            <MenuList>
              {Object.keys(SortMap).map((key, i) => {
                return (
                  <MenuItem onClick={() => onSortChange(key)} value={key} key={i}>
                    {SortMap[key]}
                  </MenuItem>
                );
              })}
