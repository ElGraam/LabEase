...
export const updateProjectMilestone = async (req: Request, res: Response, next: NextFunction) => {
  ...
  try {
    ...
    if (!existingMilestone) {
      return res.status(404).json({ message: 'マイルストーンが見つかりません' });
    }
    ...
    const updatedMilestone = await prisma.projectMilestone.update({
      ...
      data: {
        ...
        completionDate: status === ProjectMilestoneStatus.COMPLETED ? new Date() : null
      }
    });
